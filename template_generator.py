...
<body>
    ...
    # Papers页面模板（添加动态头部图片支持）
    papers_template = '''<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Paper Reviews - {{BLOG_TITLE}}</title>
    <meta name="description" content="Academic paper reviews and analysis on LLM, AI and related research">
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        :root { 
...
<body>
    <header class="header">
        <img src="{{HEADER_IMAGE}}" alt="Papers Header Background" class="header-bg-image">
        <div class="header-content">
            <h1><i class="fas fa-graduation-cap"></i> Paper Reviews</h1>
            <p class="subtitle">Academic Paper Analysis & Research Notes</p>
    ...
    <main class="main-content">
        <div class="papers-filter">
            <div class="filter-controls">
                <div class="search-box">
                    <input type="text" class="search-input" placeholder="Search papers by title or tags..." id="searchInput">
                    <i class="fas fa-search search-icon"></i>
                </div>
                <a href="#" class="filter-button active" data-category="all">All</a>
                <a href="#" class="filter-button" data-category="LLM">LLM</a>
                <a href="#" class="filter-button" data-category="NLP">NLP</a>
                <a href="#" class="filter-button" data-category="AI">AI</a>
                <a href="#" class="filter-button" data-category="ML">ML</a>
            </div>
        </div>
            ...
            button.addEventListener('click', function(e) {
                ...
                papers.forEach(paper => {
                    if (category === 'all') {
                        ...
                        const tags = Array.from(paper.querySelectorAll('.tag')).map(tag => tag.textContent);
                        const hasCategory = tags.includes(category);
                        paper.style.display = hasCategory ? 'block' : 'none';
                    }
                });
    ...
    with open(templates_dir / "papers_template.html", 'w', encoding='utf-8') as f:
        f.write(papers_template)
    print("✅ Created papers_template.html")
...
def main():
    ...
    print("2. Run: python3 blog_generator.py")
    print("3. Your blog with random images will be generated!")
